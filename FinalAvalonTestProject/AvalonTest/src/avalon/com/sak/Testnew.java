...
//	    opening a chrome browser for 1st user
		System.setProperty("webdriver.chrome.driver","chromedriver 2");
		ChromeDriver c=new ChromeDriver();
		c.manage().window().maximize();
		c.manage().timeouts().pageLoadTimeout(3,TimeUnit.SECONDS);
		
		c.get("https://chapp1.herokuapp.com/");//navigating to the given link for 1st user
		Thread.sleep(1000);
		WebElement myact1=c.findElement(By.name("name"));
		myact1.sendKeys("1st user");//sending user name for user1
		Thread.sleep(1000);
		WebElement myact2=c.findElement(By.name("room"));
		myact2.sendKeys("scenes");//sending room name for user1
		Thread.sleep(1000);
		WebElement myact3=c.findElement(By.xpath("//button[text ()='Join']"));
		myact3.click();//logging in into 1st user
	
		
//	    Opening the chrome browser of 2nd user
		...
		myac1.sendKeys("second user");//sending user name for user2
		Thread.sleep(1000);
		WebElement myac2=d.findElement(By.name("room"));
		myac2.sendKeys("scenes");//sending room name for user2
		Thread.sleep(1000);
		WebElement myac3=d.findElement(By.xpath("//button[text ()='Join']"));
		myac3.click();//logging in into 2nd user
		Thread.sleep(1000);
...
//		c.quit(); //for closing user1 browser automatically remove comment before c.quit()
//		d.quit(); //for closing user2 browser automatically remove comment before d.quit()
